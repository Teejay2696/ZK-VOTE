...
import { queryAuditLogs, exportAuditLogs, getAllAuditLogs, AuditQuery } from "../middleware/audit.js";
 ...
 */
router.get("/audit/logs", authGuard, queryLimiter, (req: Request, res: Response) => {
  const { action, actor, method, path, from, to, limit, offset } = req.query;

  const q: AuditQuery = {};
  if (action) q.action = String(action);
  if (actor) q.actor = String(actor);
  if (method) q.method = String(method);
  if (path) q.path = String(path);
  if (from) q.from = String(from);
  if (to) q.to = String(to);
  if (limit) q.limit = parseInt(String(limit), 10);
  if (offset) q.offset = parseInt(String(offset), 10);
 ...
 */
router.get("/audit/export", authGuard, queryLimiter, (req: Request, res: Response) => {
  const format = (req.query.format as string) || "json";
  if (format !== "json" && format !== "csv") {
    return res.status(400).json({ error: "format must be json or csv" });
  }

  const exported = exportAuditLogs(format as "json" | "csv");
 ...
 */
router.get("/audit/stats", authGuard, queryLimiter, (req: Request, res: Response) => {
