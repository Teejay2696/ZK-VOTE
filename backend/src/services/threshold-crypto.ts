...
export function decryptVote(
  ...
  if (g_m.equals(G1.ZERO)) return 0n;
  let m = G1_GENERATOR;
  for (let i = 1n; i < 100n; i++) {
    if (m.equals(g_m)) return i;
...
export function aggregateTally(encryptedVotes: Ciphertext[]): Ciphertext {
  if (encryptedVotes.length === 0) throw new Error("No votes to aggregate");
  return encryptedVotes.reduce((acc, vote) => homomorphicAdd(acc, vote));
}

// ── Shamir Secret Sharing ────────────────────────────────────────────

export function generatePolynomial(secret: bigint, degree: number): bigint[] {
  const coeffs: bigint[] = [secret];
  for (let i = 1; i <= degree; i++) coeffs.push(randomFr());
  return coeffs;
}
...
export function generateVSSCommitments(coeffs: bigint[]): string[] {
...
export function computeJointPublicKey(fromCommitments: string[][]): string {
...
export function decryptTally(
  ...
  if (g_tally.equals(G1.ZERO)) return 0n;
  let m = G1_GENERATOR;
  for (let i = 1n; i < 100000n; i++) {
    if (m.equals(g_tally)) return i;
    m = m.add(G1_GENERATOR);
  }
...
export function generateTallyProof(
  ciphertext: Ciphertext,
  combinedShareHex: string,
...
export function verifyTallyProof(
  ciphertext: Ciphertext,
  combinedShareHex: string,
  decryptedTally: bigint,
  _proofHex: string,
  ...
  const combinedShare = hexToG1(combinedShareHex);
  const c2 = hexToG1(ciphertext.c2);
  if (!c2.equals(combinedShare.add(g_tally))) return false;
  return true;
}
