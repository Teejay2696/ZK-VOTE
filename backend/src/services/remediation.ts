...
export const remediationActionsTotal = new Counter({
  ...
  labelNames: ["error_type", "status", "escalation"] as const,
  registers: [register],
});
...
const remediationHistory: RemediationRecord[] = [];
...
const consecutiveFailures: Record<ErrorType, number> = {
  RPC_CONNECTIVITY: 0,
  RPC_RATE_LIMITED: 0,
  SQLITE_LOCKED: 0,
  SQLITE_CORRUPT: 0,
...
let backupRpcUrls: string[] = ["https://soroban-testnet.stellar.org"];
let currentRpcIndex = 0;
let currentPollingIntervalMs = 1000;
const retryQueue: Array<{ payload: unknown; retryCount: number }> = [];

export function setBackupRpcUrls(urls: string[]): void {
  if (urls.length > 0) backupRpcUrls = urls;
}
...
export async function remediateError(
  ...
  let escalationLevel: EscalationLevel = "AUTO_REMEDIATE";
  if (failureCount >= 5) {
...
export function getRemediationHistory(limit = 50): RemediationRecord[] {
...
export function clearRemediationHistory(): void {
  remediationHistory.length = 0;
  for (const key in consecutiveFailures) {
    consecutiveFailures[key as ErrorType] = 0;
  }
}
