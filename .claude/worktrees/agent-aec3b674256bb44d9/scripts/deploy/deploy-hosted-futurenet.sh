...
# By default, treat a contract as still existing on-chain if it is
# recorded in the state file. Override CONTRACT_EXISTS_CHECK to point at
# a function that shells out to `stellar contract info` if you want a
# real on-chain existence check rather than trusting the state file.
CONTRACT_EXISTS_CHECK="${CONTRACT_EXISTS_CHECK:-}"

echo "=== Complete DaoVote Deployment Script ==="
if [ "$DRY_RUN" = "true" ]; then
  echo "*** DRY RUN MODE - no contracts will be deployed, no state will be written ***"
fi
...
if [ "$DRY_RUN" = "true" ]; then
  GUARDIAN_ADDRESS="${GUARDIAN_ADDRESS:-GDRYRUNGUARDIANADDRESSPLACEHOLDER0000000000000000}"
else
...
if [ "$DRY_RUN" = "true" ]; then
  step "[dry-run] Deployment plan:"
  echo "  Dependency order: ${DEPLOY_CONTRACT_ORDER[*]}"
  for c in "${DEPLOY_CONTRACT_ORDER[@]}"; do
    if deploy_state_is_deployed "$c"; then
      echo "  - $c: already deployed ($(deploy_state_get "$c")), would skip"
    else
      echo "  - $c: would build + deploy"
    fi
  done
  echo ""
  echo "  Would generate TypeScript bindings for each contract"
  echo "  Would create the Public DAO, init the merkle tree, and set the VK"
  echo "  Would write frontend/src/config/contracts.ts and backend/.env"
  echo ""
  success "Dry run complete - no contracts deployed, no files written"
  exit 0
fi
...
# Deploy DAO Registry
deploy_dao_registry() {
  deploy_contract "DAO Registry" "target/wasm32v1-none/release/dao_registry.wasm"
}
REGISTRY_ID=$(deploy_step "dao-registry" deploy_dao_registry)
...
deploy_membership_sbt() {
  deploy_contract "Membership SBT" "target/wasm32v1-none/release/membership_sbt.wasm" --registry "$REGISTRY_ID"
}
SBT_ID=$(deploy_step "membership-sbt" deploy_membership_sbt)
if [ -z "$SBT_ID" ]; then
...
deploy_membership_tree() {
  deploy_contract "Membership Tree" "target/wasm32v1-none/release/membership_tree.wasm" --sbt_contract "$SBT_ID" --registry "$REGISTRY_ID"
}
TREE_ID=$(deploy_step "membership-tree" deploy_membership_tree)
if [ -z "$TREE_ID" ]; then
...
# Deploy Voting
deploy_voting() {
  deploy_contract "Voting" "target/wasm32v1-none/release/voting.wasm" --tree_contract "$TREE_ID" --registry "$REGISTRY_ID" --guardian "$GUARDIAN_ADDRESS"
}
VOTING_ID=$(deploy_step "voting" deploy_voting)
if [ -z "$VOTING_ID" ]; then
...
# Deploy Comments
deploy_comments() {
  deploy_contract "Comments" "target/wasm32v1-none/release/comments.wasm" --tree_contract "$TREE_ID" --voting_contract "$VOTING_ID" --registry "$REGISTRY_ID"
}
COMMENTS_ID=$(deploy_step "comments" deploy_comments)
