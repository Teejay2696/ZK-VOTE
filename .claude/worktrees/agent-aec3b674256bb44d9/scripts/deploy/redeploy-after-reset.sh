...
if [ "$DRY_RUN" = "true" ]; then
  step "[dry-run] Would fund deployer account via friendbot"
  step "[dry-run] Would run: RPC_URL=$RPC_URL NETWORK_PASSPHRASE=$NETWORK_PASSPHRASE bash scripts/deploy/deploy-hosted-futurenet.sh --dry-run"
  bash scripts/deploy/deploy-hosted-futurenet.sh --dry-run
  step "[dry-run] Would fix networkName in frontend/src/config/contracts.ts"
  step "[dry-run] Would update backend/.env.production contract ids"
  step "[dry-run] Would rsync to $SERVER and rebuild Docker containers"
  step "[dry-run] Would verify backend/frontend health over HTTP"
  success "Dry run complete - no deployment, no file or server changes"
  exit 0
fi
